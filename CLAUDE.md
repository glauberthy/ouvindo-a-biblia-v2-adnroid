...
Release status: the app ships as a **new** app (not a reactivation of the old listing — see the `applicationId` gotcha). **`versionCode = 4` / `versionName = "1.3"` was uploaded and is IN REVIEW at the Play Console (since 2026-08-01) — submitted, not live.** It either gets approved or comes back for adjustment; don't describe the app as published, and don't assume any user has it. Operational rule: **bump `versionCode` on every Console upload, even on a test track — and the number burns on *upload*, not on publish.** Discarding the release in the Console does **not** give it back: vc3 was uploaded, discarded, and the Console then refused it with "O código de versão 3 já foi usado". That's the whole reason the numbering has holes — vc1/1.0 was the AAB validated locally when the "publish as a new app" decision was made, vc2/1.1 was built but never uploaded, vc3/1.2 was uploaded and discarded. None of them reached a user, so `docs/RELEASE_NOTES.md` still describes everything since 1.0. FASE 8 (publicação) is still open — check `PLANO_FASES_E_ISSUES.md` for what's left (mainly the 🔬 tests on the signed release APK, PUB-11/12/13/16) instead of assuming it's live.

**vc4 is burned too: the next upload must be `versionCode = 5`, and `app/build.gradle.kts` still says 4.** Bump it *before* `make release`, or the whole build finishes and only then does the Console refuse it.

**What's in review is NOT `HEAD`.** The reviewed AAB was built at `9808e58`; the tag **`v1.3-vc4`** (annotated, on `5e3213c`) marks the last commit whose *code* matches it — everything between is docs/`.gitignore`/comments. The commit after it, `c7e9b92`, is real post-submission code (R8 `optimizedResourceShrinking` ON, `statusBarColor` removed, `values-night` moved) that is **not** in the binary being reviewed. So when the Console reports something about this version, read the tag, not the working branch. Branch **`release/1.3-vc4`** is cut from the same commit and exists to take fixes if the review bounces, without dragging in what landed afterwards.

**Release marking convention** (follow it on the next upload): an annotated tag `v<versionName>-vc<versionCode>` on the exact commit that produced the uploaded AAB — its message records the artifact, the signing key, why it sits where it sits, and what changed since the last one — plus a `release/<versionName>-vc<versionCode>` branch from the same commit. `analise-media3` stays the development line (it is 213 commits ahead of `main`; `main` is stale, don't treat it as current). The older `v1.0.0-rc1` tag predates this convention.
...
make theme-light / theme-dark / theme-auto # night mode do emulador (o app segue o sistema)
```
...
**R8 only runs on `release` — `make check` cannot catch R8 breakage.** `./gradlew test` and `:app:lint` are debug-path tasks; a dependency that trips R8 compiles, tests and lints clean and then **fails the build** at `minifyReleaseWithR8`. That is how `com.google.android.play:review-ktx` landed (FASE 14): it was compiled against an older `play-services-tasks`, Cast pulls `tasks` up to 18.3.2 where `OnSuccessListener.onSuccess` carries `@NoNullnessRewrite`, and R8 aborted on the missing annotation. Fix is the `-dontwarn` AGP itself generates in `app/build/outputs/mapping/release/missing_rules.txt` — safe because that annotation is CLASS-retention, absent at runtime. **Rule: after adding or bumping any dependency, run `make release` (or at least `:app:bundleRelease`) before calling it validated.** A green `make check` proves nothing about R8.
...
- **R8 is at the recommended settings and `android.r8.optimizedResourceShrinking=true` is now ON** (ISSUE 12.B, enabled 2026-07-30): `isMinifyEnabled`, `isShrinkResources`, full mode on by AGP-8 default with no `android.enableR8.fullMode=false`, `proguard-android-optimize.txt`, and a `proguard-rules.pro` with **zero keep rules** — its one active line is a `-dontwarn` (see *R8 only runs on release* below), which silences a warning without inhibiting shrinking or optimization. The optimized shrinker is worth **−1,07 MiB / −11,6%** (AAB 9,3 → 8,3 MiB, re-confirmed on the enabling commit). **What makes it safe here, and the invariant to preserve:** it changes *which* resources ship, so a resource reached only indirectly can be dropped and fail at *runtime*, not at build. The app has **zero** runtime resource-by-name lookups (`getIdentifier`, `Resources.getSystem`, `"@drawable/…"` string literals) — every reference is static via `R`, which the shrinker can see, which is why there's no `res/raw/keep.xml`. **If you ever add a `getIdentifier` or a resource referenced only by string, declare it in a `keep.xml` before trusting the flag.** It was validated by a smoke test on the *signed release APK* (both themes: 5 tabs, covers, playback, media notification), not just a green build.
- **AGP 9.0 was deliberately NOT bumped** (the other item in the same Console panel). It needs Gradle 9.x (project is on 8.13) and is a major with breaking changes that would force re-validating Kotlin 2.0.21/KSP, Room and Hilt — for zero gain, since 9.0 only makes the flag above the *default* and it's already on. The reasoning is recorded in `gradle.properties`; treat the bump as its own task.
- **"APIs descontinuadas para exibição de ponta a ponta" is now as fixed as it can get — don't retry it** (ISSUE 12.C). Of the 5 obfuscated frames the Console listed, exactly **one** was ours (`Theme.kt`'s `window.statusBarColor`); it's gone, along with `android:statusBarColor`/`navigationBarColor` in both `themes.xml` (all no-ops under `targetSdk = 36`). The remaining callers in the release dex are `androidx.activity.EdgeToEdgeApi26`/`Api29` — i.e. the internals of `enableEdgeToEdge()`, the very call the Console recommends — plus androidx.core and Material. **So the warning will keep showing up and that's expected.** To re-check after a dependency bump, de-obfuscate via `app/build/outputs/mapping/release/mapping.txt` instead of guessing what a frame like `d.r.b` means.
...
- **O Android Studio sobe este mesmo AVD com `-qt-hide-window`** — a aba *Running Devices*, embutida na IDE. Ou seja o emulador pode estar **de pé e booted sem nenhuma janela na tela**, e aí `make emulator` (ou `emulator -avd …` na mão) morre com `FATAL | Running multiple emulators with the same AVD is an experimental feature. Please use -read-only flag`. Não é lock órfão e o `-read-only` não é o que se quer: o AVD simplesmente já está rodando. Confirme com `pgrep -af "qemu-system.*-avd OuvindoBiblia_API36"` (o pai é o `com.android.tools.idea.MainWrapper`) ou `adb devices`, e use o `emulator-5554` que já existe. `make emulator` agora detecta isso e vira no-op; `make emulator-kill` derruba. O `ERROR | Your GPU cannot be used for hardware rendering` que aparece nesse log é ruído — o check seguinte diz `hasSufficientHwGpu: Ok`.
...
- **Dark theme is ON** (`DARK_THEME_ENABLED = true` in `ui/theme/Theme.kt`, following `isSystemInDarkTheme()`; both schemes were checked screen by screen on the emulator). The flag exists as a regression brake: if a new screen hard-codes light colors, turning it off beats shipping a half-cream/half-dark app. **There is no in-app theme picker** — to see the other scheme, flip the *system* night mode (`make theme-light` / `theme-dark` / `theme-auto`, i.e. `adb shell cmd uimode night no|yes|auto`), which recomposes live with no reinstall. Don't reach for `DARK_THEME_ENABLED` to do that: it forces light *always*, ignoring the system.
...
- **A resource-qualifier directory must be a direct child of `res/`, and putting it deeper fails *silently*.** `res/values/values-night/themes.xml` sat there unnoticed (ISSUE 12.D, fixed 2026-07-30): the merger never compiled it, so dark mode kept the *light* `windowBackground`, **and** its dangling `@color/dark_background` reference never errored because the file was never read. Two bugs hiding each other, zero build output. `res/values-night/themes.xml` is the correct path. Related: a `values-night` style **replaces** the whole style, it doesn't merge item-by-item — `windowNoTitle`/`windowActionBar` have to be repeated in the night variant. And `dark_background` in `colors.xml` mirrors `DarkBackground` in `ui/theme/Color.kt`; change one, change the other.
